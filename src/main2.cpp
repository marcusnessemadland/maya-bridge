...
static SharedBufferI* s_buffer = NULL;
static SharedBufferI* s_readbuffer = NULL;
...
void callbackTimer(float _elapsedTime, float _lastTime, void* _clientData)
{
	...
	if (status & MAYABRIDGE_MESSAGE_RECEIVED)
	{
		...
		if (!s_meshRemovedQueue.empty())
		{
			...
			//
			strcpy(meshEvent.m_name, name.asChar());
		...
		else if (!s_meshChangedQueue.empty())
		{
			...
			if (status == MS::kSuccess)
			{
				...
				if (meshEvent.m_numVertices <= MAYABRIDGE_CONFIG_MAX_VERTICES)
				{
				...
				if (numNormals <= MAYABRIDGE_CONFIG_MAX_VERTICES)
				{
				...
				if (numUVs <= MAYABRIDGE_CONFIG_MAX_VERTICES)
				{
				...
				if (meshEvent.m_numIndices <= MAYABRIDGE_CONFIG_MAX_INDICES)
				{
		...
		if (!s_transformChangedQueue.empty() && s_meshChangedQueue.empty())
		{
			...
			//
			strcpy(transformEvent.m_name, name.asChar());
		...
		if (!s_materialChangedQueue.empty() && s_meshChangedQueue.empty())
		{
			...
			//
			strcpy(materialEvent.m_name, name.asChar());
								...
								for (unsigned int j = 0; j < connectedPlugs.length(); ++j)
								{
									...
									if (connectedNodeFn.typeName() == "file")
									{
										...
										if (!filePathPlug.isNull())
										{
											...
											strcpy(materialEvent.m_diffusePath, textureFilePath.asChar());
										...
										if (!bumpValuePlug.isNull() && bumpValuePlug.isConnected())
										{
											...
											for (unsigned int k = 0; k < connectedPlugs.length(); ++k)
											{
												...
												if (bumpConnectedNodeFn.typeName() == "file")
												{
													...
													{
														MString textureFilePath = filePathPlug.asString();
														strcpy(materialEvent.m_normalPath, textureFilePath.asChar());
							...
							if (roughnessPlug.isConnected())
							{
								...
								for (unsigned int j = 0; j < connectedPlugs.length(); ++j)
								{
									...
									if (connectedNodeFn.typeName() == "file")
									{
										...
										{
											MString textureFilePath = filePathPlug.asString();
											strcpy(materialEvent.m_roughnessPath, textureFilePath.asChar());
							...
							if (metallicPlug.isConnected())
							{
								...
								for (unsigned int j = 0; j < connectedPlugs.length(); ++j)
								{
									...
									if (connectedNodeFn.typeName() == "file")
									{
										...
										{
											MString textureFilePath = filePathPlug.asString();
											strcpy(materialEvent.m_metallicPath, textureFilePath.asChar());
